...
# Restore read-only. Don't suppress with `||` — if this fails, set -e aborts
# and the trap re-asserts readonly=on so /usr is never left writable until
# reboot. Mirrors install.sh's matching block (no `||`, no swallowing).
